   ...
   Subshader 
   {
      Pass 
      {
         ...
         FragInput vert ( VertexInput vi )
         {
            ...
            fi.uv = TRANSFORM_TEX ( vi.uv, _NoiseTex );   // this will use texture offsets and stuff like that 
            ...
            {
               float vertexToCustomValueDist = 0;//ConvertToNewRange( 0.7, 1, 0.0, 2, random( fi.uv * floor(_Time.y * 3 ) ) ) ;

               float rand1 = random( fi.uv * floor(_Time.y * 3 ) );
               if ( rand1 >0.9 && rand1 < 1 ) 
               vertexToCustomValueDist = 1;
               else 
               vertexToCustomValueDist = 0;
